...
applied in-flight at push time (the existing `git commit-tree`-based
re-authoring path, which rebuilds each commit's object with the new
identity rather than checking anything out) and persisted nowhere — no
local git config is written, on any host or in any repo, to produce this.
